...
```{r}
# Then, do the same manually using MLE - complete the code
...
m_linear_ipw <- lm_robust(
  fte ~ nj + , # Fill in with the same covariates you used
  data = ck0,
  weights = weight
)
m_linear_ipw
...
mu_1 <- lm_robust(
  fte ~ , # Fill in with the same covariates you used
...
```{r}
# Recover the regression-adjusted ATE using the Lin estimator
lin_ate <- lm_lin(
  fte ~ nj,
  covariates = ~ , # Fill in with the same covariates you used
  data = ck0
)
