...
plot_boot <- ggplot(boot_long, aes(x = estimator, y = estimate, fill = estimator)) +
  geom_violin(alpha = 0.75, linewidth = 0.3, trim = FALSE) +
...
%-------------------------------------------------------------------------------%
\begin{frame}{Bootstrap summaries + MC CI coverage}
\begin{center}
...
summary_tbl <- boot_df |>
  ...
  mutate(
    ...
    estimator = factor(estimator,
                        ...
                        labels = c("MCAR", "MAR", "Manski lo", "Manski hi"))
  ) |>
  arrange(mechanism, estimator)

# Keep the coverage simulation moderate so the mini deck recompiles quickly.
S <- 80
B_cover <- 250
...
\nocite{holland_1986, rubin_1976, manski_2003b, hernan-robins_2010}
