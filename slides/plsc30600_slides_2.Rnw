...
\draw[->, line width=0.6pt] (0,0) -- (0,1.05) node[left] {$\E[Y \mid D=d, X]$};
...
\[
m(d,x) \;=\; \alpha \;+\; \beta d^{1/2} \;+\; \gamma d^2 \;+\; \eta x d.
\]
\pause
...
\pause
\vspace{0.5em}
\textit{Note: for $D\sim \mathrm{Beta}(2,2)$, the density is $f_D(d)=6d(1-d)$ on $(0,1)$.}
\end{frame}
...
= 0 + \beta \frac{1}{2} d^{-1/2} + 2\gamma d + \eta x
= \frac{\beta}{2\sqrt d} + 2\gamma d + \eta x.
\]\pause
\medskip
\textbf{Step 2: Use linearity of expectation.}
\[
\mathrm{AMCE}
= \E\!\left[\frac{\beta}{2\sqrt D}+2\gamma D+\eta X\right]
= \frac{\beta}{2}\E[D^{-1/2}] + 2\gamma \E[D] + \eta \E[X].
\]\pause
...
%-------------------------------------------------------------------------------%
\begin{frame}{AMCE}
\textbf{Step 3: Compute the needed moments for $D\sim \mathrm{Beta}(2,2)$.}
...
\underline{Compute $\E[D^{-1/2}]$.}
\[
\E[D^{-1/2}]
=\int_0^1 u^{-1/2} f_D(u)\,du
=\int_0^1 u^{-1/2} 6u(1-u)\,du
=6\int_0^1 u^{1/2}(1-u)\,du.
\]
\[
=6\int_0^1 \left(u^{1/2}-u^{3/2}\right)\,du
=6\left(\int_0^1 u^{1/2}\,du-\int_0^1 u^{3/2}\,du\right).
\]
Using $\int_0^1 u^a\,du=\frac{1}{a+1}$ for $a>-1$:
\[
\int_0^1 u^{1/2}\,du=\frac{2}{3},
\qquad
\int_0^1 u^{3/2}\,du=\frac{2}{5}.
\]
Thus,
\[
\E[D^{-1/2}]
=6\left(\frac{2}{3}-\frac{2}{5}\right)
=\frac{8}{5}.
\]
\end{frame}
%-------------------------------------------------------------------------------%
...
\[
\mathrm{AMCE}
=\frac{\beta}{2} \frac{8}{5} + 2\gamma \frac12 + \eta p
=\frac{4}{5}\beta + \gamma + \eta p.
\]

\[
\boxed{\mathrm{AMCE}=\frac{4}{5}\beta+\gamma+\eta p.}
\]
\end{frame}
