...
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{c|c|c|c|c|c|c|c}
$i$ & $X_{[1]i}$ & $X_{[2]i}$ & $\hat p_D(\X_i)$ & $Y_i(0)$ & $Y_i(1)$ & \textcolor{Violator1}{$D_i$} & \textcolor{Violator1}{$Y_i$} \\
\cmidrule(lr){1-1}\cmidrule(lr){2-6}\cmidrule(lr){7-8}
...
%-------------------------------------------------------------------------------%
\begin{frame}{IPW in matrix notation}
\small

\[
\XX=\begin{bmatrix}\X_1^\top\\ \vdots\\ \X_n^\top\end{bmatrix}\in\mathbb{R}^{n\times k},\quad
\Y=\begin{bmatrix}Y_1\\ \vdots\\ Y_n\end{bmatrix}\in\mathbb{R}^{n},\quad
\D=\begin{bmatrix}D_1\\ \vdots\\ D_n\end{bmatrix}\in\{0,1\}^{n},\quad
\one=\begin{bmatrix}1\\ \vdots\\ 1 \end{bmatrix}\in\mathbb{R}^{n}.
\]
...
\[
\W_1=\operatorname{diag}\!\big(\D\oslash \hat{\p}\big),\qquad
\W_0=\operatorname{diag}\!\big((\one-\D)\oslash(\one-\hat{\p})\big).
\]
i.e., 
$\mathrm{diag}(\cdot)$ places the weights on the diagonal so each $Y_i$ is reweighted separately.
\scriptsize
\[
\W_1=
\begin{bmatrix}
\frac{D_1}{\hat p_D(\X_1)} & 0 & \cdots & 0\\
0 & \frac{D_2}{\hat p_D(\X_2)} & \cdots & 0\\
\vdots & \vdots & \ddots & \vdots\\
0 & 0 & \cdots & \frac{D_n}{\hat p_D(\X_n)}
\end{bmatrix},
\quad
\W_0=
\begin{bmatrix}
\frac{1-D_1}{1-\hat p_D(\X_1)} & 0 & \cdots & 0\\
0 & \frac{1-D_2}{1-\hat p_D(\X_2)} & \cdots & 0\\
\vdots & \vdots & \ddots & \vdots\\
0 & 0 & \cdots & \frac{1-D_n}{1-\hat p_D(\X_n)}
\end{bmatrix}.
\]
...
\[
\hat\tau_{\mathrm{IPW}}
=\frac{1}{n}\,\one^\top (\W_1-\W_0)\Y
=\frac{1}{n}\sum_{i=1}^n\left(\frac{D_iY_i}{\hat p_D(\X_i)}-\frac{(1-D_i)Y_i}{1-\hat p_D(\X_i)}\right).
\]

% (Optional) normalized/H\'ajek version:
% \[
% \hat\tau_{\mathrm{IPW}}^{H}
% =\frac{\one^\top W_1\Y}{\one^\top W_1\one}
% -\frac{\one^\top W_0\Y}{\one^\top W_0\one}.
% \]
